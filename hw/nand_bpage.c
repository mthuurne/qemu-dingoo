/*
 * Big page NAND flash memory emulation.
 * based on 256M/16 bit flash datasheet from micro(MT29F2G16ABC)
 *
...
struct nand_flash_info_s nand_flash_info[] = {
    { NAND_MFR_MICRON,  0xba,  256, BUS_WIDTH_16, 11, 6, 6 },
    { NAND_MFR_SAMSUNG, 0xd3, 1024, BUS_WIDTH_8,  11, 6, 6 },
    { NAND_MFR_SAMSUNG, 0xd5, 2048, BUS_WIDTH_8,  12, 7, 7 },
    { NAND_MFR_SAMSUNG, 0xd7, 4096, BUS_WIDTH_8,  12, 7, 7 },
};
...
static void nand_blk_write(struct nand_bflash_s *s)
{
    ...
    debug_out(s, "nand_blk_write page number %x s->addr_low "
...
static void nandb_blk_load(struct nand_bflash_s *s)
{
    ...
    page_number = get_page_number(s, s->addr_low, s->addr_high);
    debug_out(s, "nandb_blk_load page number %x s->addr_low "
              "%x s->addr_high %x\n", page_number, s->addr_low, s->addr_high);
    if (page_number >= s->pages)
        return;
...
static void nandb_blk_erase(struct nand_bflash_s *s)
{
    ...
    i = (addr & ~0x1ff) + 0x200;
    addr += s->page_oob_size*s->block_pages - 0x200;
    for (; i < addr; i += 0x200) {
